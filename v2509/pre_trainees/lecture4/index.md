...
<center>The mechine is always right</center>

<center>难度指数：015</center>

<center>报酬：专业程序员基础（3）</center>
...
不仅如此，我们还加了几条注释，将最终程序的行为和某些汇编代码联系了起来（比如赋值操作实际就是内存写入），这也有助于你理解[上一节讲义开头提到的内存分配顺序](/v2509/pre_trainees/lecture3/指针_结构体教程.md)。当然前提还是你要能读懂汇编代码。如果现在就你对此感兴趣可以尝试使用这个网站：[Compiler Explorer](https://godbolt.org/)
...
>     1. 收集用户输入的名字和年龄，然后存入结构体中。规定输入的名字大小不能超过19个字节。
> 
>     2. 结构体中有两个成员：长度为20字节的char数组作为名字储存和int类型的变量作为年龄储存
> 
>     3. 将获得的<strong>结构体</strong>的数据用fwrite写入当前目录下的build/list.dat文件路径，如果没有build路径记得自己创建
> 
> 
> 2. 你还需要实现另一个指令：`list`
>     1. 执行后会将所有数据库（build/list.dat）中的数据进行解析输出，每行输出的格式为：
> 
> 
